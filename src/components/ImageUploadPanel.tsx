...
export function ImageUploadPanel({
  ...
  const handleCreateModel = () => {
    if (!file || isGenerating || isComplete) return;
    onCreateModel?.();
  };

  const isCreateDisabled = !file || isGenerating || isComplete;
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start justify-between gap-3 rounded-lg border border-border bg-background px-4 py-3">
            ...
            <button
              ...
              className="flex shrink-0 items-center gap-1.5 rounded-full border border-border px-3 py-1.5 text-xs font-medium text-muted transition-colors hover:border-primary/30 hover:text-primary"
            >
